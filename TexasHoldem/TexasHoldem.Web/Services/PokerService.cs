...
namespace TexasHoldem.Web.Services
{
    public class PokerService
    {
        ...
        public IEnumerable<FinalPoker> Summary(List<FinalPoker> finalPokers)
        {
            ...
            {
                var result = float.Parse(Compute(item));
                item.Result = result;
            }
        ...
        public string Compute(FinalPoker finalPoker)
        {
            ...
            var groupeds = finalPoker.Pokers.OrderBy(it => it.PokerSize).GroupBy(it => it.PokerSize);
            foreach (var item in groupeds)
            {
                int count = 0;
                ...
                singleCards.Add(new SingleCard(item.Key, count));
            }
            ...
            if (single != null)
            {
                ...
                //三条
                another = singleCards.LastOrDefault(it => it.PokerSize != single.PokerSize);
            ...
            if (sum_C >= 5)
            {
                ...
                return "6." + pokers[4].PokerSize + pokers[3].Value + pokers[2].Value + pokers[1].Value + pokers[0].Value;
            }
            ...
            if (singleCards.Count == 5)
            {
                ...
                return "3." + single.Value + another.Value;
            }
            //一对
            if (singleCards.Count == 6)
            {
                single = singleCards.LastOrDefault(it => it.Count == 2);
                return "2." + single.Value;
            }
            //高牌
